...
#' @export
`$<-.savvy_rpkg__sealed` <- function(x, name, value) {
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}

#' @export
`[[<-.savvy_rpkg__sealed` <- function(x, i, value) {
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}
...
`.savvy_wrap_Person` <- function(ptr) {
  ...
  class(e) <- c("Person", "savvy_rpkg__sealed")
  e
}
...
class(`Person`) <- c("Person__bundle", "savvy_rpkg__sealed")
